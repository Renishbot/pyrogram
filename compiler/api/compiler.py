...
# noinspection PyShadowingBuiltins
def start_fast(format: bool = False):
    shutil.rmtree(DESTINATION_PATH / "types", ignore_errors=True)
    shutil.rmtree(DESTINATION_PATH / "functions", ignore_errors=True)
    shutil.rmtree(DESTINATION_PATH / "base", ignore_errors=True)
    ...
    with open(HOME_PATH / "template/type_fast.txt") as f1, \
            open(HOME_PATH / "template/combinator_fast.txt") as f2:
        type_tmpl = f1.read()
        combinator_tmpl = f2.read()
    ...
    for combinator in combinators:
        qualtype = combinator.qualtype

        if qualtype.startswith("Vector"):
            qualtype = qualtype.split("<")[1][:-1]

        d = types_to_constructors if combinator.section == "types" else types_to_functions

        if qualtype not in d:
            d[qualtype] = []

        d[qualtype].append(combinator.qualname)

        if combinator.section == "types":
            key = combinator.namespace

            if key not in namespaces_to_types:
                namespaces_to_types[key] = []

            if combinator.type not in namespaces_to_types[key]:
                namespaces_to_types[key].append(combinator.type)
    ...
    for qualtype in types_to_constructors:
        typespace, type = qualtype.split(".") if "." in qualtype else ("", qualtype)
        dir_path = DESTINATION_PATH / "base"

        module = typespace or '__init__'
        ...
        with open(dir_path / f"{module}.py", "a") as f:
            if module not in header_inserted:
                header_inserted.append(module)
                f.write(
                    f"{notice}\n\n"
                    f"{WARNING}\n\n"
    ...
    header_inserted.clear()

    for combinator in combinators:
        sorted_args = sort_args(combinator.args)

        arguments = (
                (", *, " if combinator.args else "") +
                (", ".join(
                    [f"{i[0]}: {get_type_hint(i[1])}"
                     for i in sorted_args]
                ) if sorted_args else "")
        ...
        for i, arg in enumerate(sorted_args):
            ...
            docstring_args.append(
                "{}{}: {}".format(
                    arg_name,
                    " (optional)".format(flag_number) if is_optional else "",
                    get_docstring_arg_type(arg_type, is_pyrogram_type=combinator.namespace == "pyrogram")
                )
            )

        if combinator.section == "types":
            docstring += f"This object is a constructor of the base type :obj:`~pyrogram.raw.base.{combinator.qualtype}`.\n\n"
        else:
            docstring += f"Telegram API method.\n\n"

        docstring += f"    Details:\n        - Layer: ``{layer}``\n        - ID: ``{combinator.id[2:].upper()}``\n\n"
        ...
        if combinator.section == "functions":
            docstring += "\n\n    Returns:\n        " + get_docstring_arg_type(combinator.qualtype)
        else:
            references, count = get_references(combinator.qualname, "constructors")
        ...
        write_types = read_types = "" if combinator.has_flags else "# No flags\n        "

        for arg_name, arg_type in combinator.args:
            flag = FLAGS_RE_2.match(arg_type)

            if re.match(r"flags\d?", arg_name) and arg_type == "#":
                write_flags = []

                for i in combinator.args:
                    flag = FLAGS_RE_2.match(i[1])
            ...
            if flag:
                ...
                if flag_type == "true":
                    ...
                    write_types += "\n        "
                    write_types += f"self.{arg_name} and b.write({flag_type.title()}(self.{arg_name}))\n        "
        ...
        directory = combinator.section

        dir_path = DESTINATION_PATH / directory

        os.makedirs(dir_path, exist_ok=True)

        module = combinator.namespace or '__init__'

        with open(dir_path / f"{module}.py", "a") as f:
            if directory + module not in header_inserted:
                header_inserted.append(directory + module)
                f.write(
                    f"{notice}\n\n"
                    f"{WARNING}\n\n"
            ...
            f.write(combinator_tmpl.format(
                ...
                slots=slots,
                id=combinator.id,
                qualname=f"{combinator.section}.{combinator.qualname}",
                arguments=arguments,
                fields=fields,
                read_types=read_types,
                write_types=write_types,
        ...
        d = namespaces_to_constructors if combinator.section == "types" else namespaces_to_functions

        if combinator.namespace not in d:
            d[combinator.namespace] = []

        d[combinator.namespace].append(combinator.name)

    for lists, namespaces in (
            (namespaces_to_types, "base"),
            (namespaces_to_constructors, "types"),
            (namespaces_to_functions, "functions")
    ):
        with open(DESTINATION_PATH / namespaces / "__init__.py", "a") as f:
            f.write(f"from . import {', '.join(filter(bool, lists))}")

    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        f.write(
            f"{notice}\n\n"
            f"{WARNING}\n\n"
            f"from pyrogram.raw import core, {', '.join(set(c.section for c in combinators))}\n\n"
            f"layer = {layer}\n\n"
            "objects = {\n" +
            "\n".join(tuple(
                f'    {combinator.id.ljust(10)}: {combinator.section}.{combinator.qualname},' for combinator in
                combinators
            )) +
...
if "__main__" == __name__:
    ...
    (start if len(argv) > 1 and "orig" in argv[1].lower() else start_fast)(format=False)
